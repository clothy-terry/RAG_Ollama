        ...
        for sentence in doc.sents:
            for keyword in keywords:
                if keyword in sentence.text.lower():
                    context_words = [token.text for token in sentence if token.text.lower() not in stopwords.words('english')]
                    if keyword in keyword_context:
                        keyword_context[keyword].append((sentence.text, context_words))
                    else:
                        keyword_context[keyword] = [(sentence.text, context_words)]
    # find context
    else:
        for token in doc:
            if token.text.lower() in keywords:
                ...
                if token.text.lower() in keyword_context:
                    keyword_context[token.text.lower()].append(context_words)
                else:
                    keyword_context[token.text.lower()] = [context_words]
...
keyword_context = find_keyword_context(text, custom_keywords)
